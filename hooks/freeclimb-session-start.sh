...
if [ ! -f "$plugin_root/mcp/lib/bin.js" ]; then
  cat <<'JSON'
{"additional_context":"FreeClimb plugin: the local packages are not built yet, so MCP inspection, automatic PerCL validation, and the bundled CLI will not work. Run the /freeclimb-setup command once (it runs `pnpm run setup` in the plugin directory to install dependencies and build core/mcp/cli). Skills, rules, and commands still work without it."}
JSON
  exit 0
fi
