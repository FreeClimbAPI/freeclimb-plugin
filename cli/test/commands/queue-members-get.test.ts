...
describe("queue-members:get Data Test", function () {
    const testJson = {
  ...
  "message": "Response from server"
    ...
    describe("queue-members:get next flag test", function () {
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            try {
                delete process.env.FREECLIMB_QUEUE_MEMBERS_GET_NEXT
                nock("https://www.freeclimb.com")
                    .get(
                        `/apiserver/Accounts/${await cred.accountId}/Queues/${queueId}/Members/${callId}`
                    )
...
describe("queue-members:get Status Test", function () {
