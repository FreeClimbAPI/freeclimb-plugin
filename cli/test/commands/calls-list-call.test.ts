...
describe("calls:list-call Data Test", function () {
    const testJson = {
  ...
  "message": "Response from server"
    ...
    describe("calls:list-call query param flags", function () {
        afterEach(() => {
            nock.cleanAll()
        })
        ...
        it("required params and a query param is sent through with request-dateCreated", async () => {
            nock("https://www.freeclimb.com")
                .get(`/apiserver/Accounts/${await cred.accountId}/Calls/${callId}/Recordings`)
                .query({ dateCreated: "userInput-dateCreated" })
                .reply(200, testJson)
    ...
    describe("calls:list-call next flag test", function () {
        const testJsonNext = {
            total: 1,
            start: 0,
            end: 0,
        ...
        before(async () => {
            ...
            testJsonNext2.nextPageUri = `https://www.freeclimb.com/apiserver/Accounts/${accountId}/Calls/${callId}/Recordings?cursor=freeClimbCLIAutomatedTestCursor`
        })
        ...
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            ...
            delete process.env.FREECLIMB_CALLS_LIST_CALL_NEXT
            try {
                await runCommand(["calls:list-call", "userInput-callId"])
                const { error } = await runCommand([
                    "calls:list-call",
                    "userInput-callId",
                    "--next",
...
describe("calls:list-call Status Test", function () {
