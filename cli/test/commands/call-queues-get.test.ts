...
describe("call-queues:get Data Test", function () {
    const testJson = {
  ...
  "message": "Response from server"
    ...
    it("Test all required paramaters", async () => {
        nock("https://www.freeclimb.com")
            .get(`/apiserver/Accounts/${await cred.accountId}/Queues/${queueId}`)
            .query({})
            .reply(200, testJson)
        const { stdout } = await runCommand(["call-queues:get", "userInput-queueId"])
    ...
    it("Test Freeclimb Api error repsonce is process correctly without a suggestion", async () => {
        ...
        const { error } = await runCommand(["call-queues:get", "userInput-queueId"])
    ...
    describe("call-queues:get next flag test", function () {
        afterEach(() => {
            nock.cleanAll()
        })
        ...
        it("Tests return of Exit Code 3 when flag next is not available", async () => {
            nock("https://www.freeclimb.com")
                ...
                .query({})
                .reply(200, testJson)
            const orig = process.env.FREECLIMB_CALL_QUEUES_GET_NEXT
            delete process.env.FREECLIMB_CALL_QUEUES_GET_NEXT
            try {
                await runCommand(["call-queues:get", "userInput-queueId"])
                const { error } = await runCommand([
                    "call-queues:get",
                    "userInput-queueId",
                    "--next",
                ...
                if (orig === undefined) {delete process.env.FREECLIMB_CALL_QUEUES_GET_NEXT}
                else {process.env.FREECLIMB_CALL_QUEUES_GET_NEXT = orig}
            }
        })
    })
})

describe("call-queues:get Status Test", function () {
