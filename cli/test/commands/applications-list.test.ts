...
describe("applications:list Data Test", function () {
    ...
    it("Test all required paramaters", async () => {
        nock("https://www.freeclimb.com")
            ...
            .query({})
            .reply(200, testJson)
        const { stdout } = await runCommand(["applications:list", "--json"])
    ...
    describe("applications:list next flag test", function () {
        ...
        before(async () => {
            ...
            testJsonNext2.nextPageUri = `https://www.freeclimb.com/apiserver/Accounts/${accountId}/Applications?cursor=freeClimbCLIAutomatedTestCursor`
        })
        ...
        it("Test flag next works as expected when available with on last page", async () => {
            ...
            try {
                const { stdout } = await runCommand(["applications:list", "--next", "--json"])
                expect(stdout).to.contain('"page": 1')
            } finally {
                if (orig === undefined) {delete process.env.FREECLIMB_APPLICATIONS_LIST_NEXT}
                else {process.env.FREECLIMB_APPLICATIONS_LIST_NEXT = orig}
            }
        })
