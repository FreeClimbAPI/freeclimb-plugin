...
```
USAGE
  $ freeclimb calls:get CALLID [--json] [--quiet] [--fields <value>] [-h]
...
EXAMPLES
  $ freeclimb calls:get CA1234567890abcdef

  $ freeclimb calls:get CA1234567890abcdef --json

  $ freeclimb calls:get CA1234567890abcdef --fields callId,status,from,to
```

_See code: [src/commands/calls/get.ts](https://github.com/FreeClimbAPI/freeclimb-plugin/blob/v0.6.0/src/commands/calls/get.ts)_
...
```
USAGE
  $ freeclimb calls:list [-T <value>] [-f <value>] [-S <value>] [-s <value>] [-e <value>] [-P <value>] [-a
    true|false] [-n] [--json] [--quiet] [--fields <value>] [-h]

FLAGS
  -P, --parentCallId=<value>  Only show Calls generated by the call with this ID.
  -S, --status=<value>        Only show Calls currently in this status. May be queued, ringing, inProgress, canceled,
                              completed, failed, busy, or noAnswer.
  -T, --to=<value>            Only show Calls to this phone number.
  -a, --active=<option>       Flag defaults to false. Set true to list active calls.
                              <options: true|false>
  -e, --endTime=<value>       Only show Calls that ended at or before this time, given as YYYY-MM- DD hh:mm:ss.
  -f, --from=<value>          Only show Calls from this phone number.
  -h, --help                  Show CLI help.
  -n, --next                  Displays the next page of output.
  -s, --startTime=<value>     Only show Calls that started at or after this time, given as YYYY-MM-DD hh:mm:ss.
...
EXAMPLES
  ...
  $ freeclimb calls:list --status ringing --json

  $ freeclimb calls:list --quiet

  $ freeclimb calls:list --fields callId,status,from,to
```

_See code: [src/commands/calls/list.ts](https://github.com/FreeClimbAPI/freeclimb-plugin/blob/v0.6.0/src/commands/calls/list.ts)_
...
```
USAGE
  $ freeclimb calls:list-call [CALLID] [-d <value>] [-n] [--json] [-h]

ARGUMENTS
  [CALLID]  String that uniquely identifies this call resource.

FLAGS
  -d, --dateCreated=<value>  Only show recordings created on the specified date, in the form YYYY-MM-DD.
  -h, --help                 Show CLI help.
  -n, --next                 Displays the next page of output.
...
_See code: [src/commands/calls/list-call.ts](https://github.com/FreeClimbAPI/freeclimb-plugin/blob/v0.6.0/src/commands/calls/list-call.ts)_
...
```
USAGE
  $ freeclimb calls:list-call-logs [CALLID] [-n] [--json] [-h]
...
_See code: [src/commands/calls/list-call-logs.ts](https://github.com/FreeClimbAPI/freeclimb-plugin/blob/v0.6.0/src/commands/calls/list-call-logs.ts)_
...
```
USAGE
  $ freeclimb calls:make FROM TO APPLICATIONID [-c <value>] [-P <value>] [-s <value>] [-i <value>] [-I <value>]
    [-t <value>] [-p true|false] [--json] [--quiet] [--fields <value>] [--dry-run] [-h]
...
FLAGS
  ...
  --fields=<value>
      Comma-separated list of fields to include in the response. Limits output to protect context windows when used by
      agents.
...
DESCRIPTION
  Making a Call may take time. A 202 status code is returned if the Call request was successfully queued by FreeClimb,
  otherwise, a 500 error is returned. The asynchronous callback for the result will occur after some time through the
  callConnectUrl.

EXAMPLES
  $ freeclimb calls:make +12223334444 +15556667777 AP1234567890abcdef1234567890abcdef12345678 https://example.com/voice

  $ freeclimb calls:make +12223334444 +15556667777 AP1234567890abcdef1234567890abcdef12345678 https://example.com/voice --dry-run
```

_See code: [src/commands/calls/make.ts](https://github.com/FreeClimbAPI/freeclimb-plugin/blob/v0.6.0/src/commands/calls/make.ts)_
...
```
USAGE
  $ freeclimb calls:update CALLID STATUS [--json] [--quiet] [--fields <value>] [--dry-run] [-h]
...
DESCRIPTION
  Call hang up may take time. A 202 status code is returned if the hangup request was successfully queued by FreeClimb.
  Otherwise, an error code is returned. If successfully queued, the asynchronous callback for the result will occur
  after some time through the statusCallbackUrl.

EXAMPLES
  $ freeclimb calls:update CA1234567890abcdef --status completed

  $ freeclimb calls:update CA1234567890abcdef --status completed --dry-run
```

_See code: [src/commands/calls/update.ts](https://github.com/FreeClimbAPI/freeclimb-plugin/blob/v0.6.0/src/commands/calls/update.ts)_
