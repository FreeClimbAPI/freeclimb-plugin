...
export class applicationsUpdate extends Command {
    ...
    async run() {
        ...
        if (flags.voiceUrl) {
            rejectControlChars(flags.voiceUrl, "voiceUrl")
        }
        if (flags.voiceFallbackUrl) {
            rejectControlChars(flags.voiceFallbackUrl, "voiceFallbackUrl")
        }
        ...
        if (flags.smsUrl) {
            rejectControlChars(flags.smsUrl, "smsUrl")
        }
        if (flags.smsFallbackUrl) {
            rejectControlChars(flags.smsFallbackUrl, "smsFallbackUrl")
        }
        if (flags["dry-run"]) {
            const dryRunOutput = {
                dryRun: true,
        ...
        const formatOutput = (data: any) => {
            ...
            out.render(outputData, { topic: "applications", command: "update" })
            return null
        }
