...
export class callsList extends Command {
    ...
    async run() {
        ...
        if (flags.parentCallId) {
            validateResourceId(flags.parentCallId, "parentCallId")
        }
        if (flags.to) {
        ...
        if (flags.status) {
            rejectControlChars(flags.status, "status")
        }
        if (flags.startTime) {
            rejectControlChars(flags.startTime, "startTime")
        }
        if (flags.endTime) {
        ...
        const formatOutput = (data: any) => {
            ...
            out.render(outputData, { topic: "calls", command: "list" })
            return null
        }
                ...
                if (outputFormat === "json") {
                    out.out(
                        JSON.stringify(
                            wrapJsonOutput(null, {
                                command: "calls:list",
                                request: { method: "GET", endpoint: `Calls` },
                            }),
                            null,
                            2,
                        ),
