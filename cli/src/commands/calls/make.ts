...
export class callsMake extends Command {
    ...
    async run() {
        ...
        if (flags.callConnectUrl) {
            rejectControlChars(flags.callConnectUrl, "callConnectUrl")
        }
        if (flags.sendDigits) {
            rejectControlChars(flags.sendDigits, "sendDigits")
        }
        if (flags.ifMachine) {
            rejectControlChars(flags.ifMachine, "ifMachine")
        }
        if (flags.ifMachineUrl) {
            rejectControlChars(flags.ifMachineUrl, "ifMachineUrl")
        }
        if (flags["dry-run"]) {
            const dryRunOutput = {
                dryRun: true,
                ...
                body: {
                    ...
                    privacyMode:
                        typeof flags.privacyMode === "undefined"
                            ? undefined
                            : flags.privacyMode === "true",
                },
            }
