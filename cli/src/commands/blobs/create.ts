...
export class blobsCreate extends Command {
    static description = ` Create a new Blob within the specified account.`
    static examples = [
        '<%= config.bin %> blobs:create --data \'{"alias":"my-state","value":1}\'',
        '<%= config.bin %> blobs:create --data \'{"alias":"my-state"}\' --dry-run',
    ]

    static flags = {
        data: Flags.string({
            char: "d",
            description: "Blob payload as a JSON string.",
            required: true,
        }),
    ...
    async run() {
        const spec: CommandSpec = {
            ...
            quietIdKey: "blobId",
            dryRun: true,
            validations: [{ source: "flags", key: "data", rule: "controlChars" }],
