...
${FREECLIMB_THEME_CSS}
body { padding:4px; }
#app { background:linear-gradient(135deg,var(--background-blue),var(--background-dark)); border:1px solid var(--faint); border-radius:14px; box-shadow:0 18px 50px rgba(0,0,0,.28); display:flex; flex-direction:column; gap:16px; overflow:hidden; padding:20px; }
.snapshot { border-bottom:1px solid var(--faint); display:flex; justify-content:space-between; gap:12px; color:var(--muted); font-size:12px; padding-bottom:12px; }
.snapshot strong { color:var(--brand-green); letter-spacing:.08em; text-transform:uppercase; }
.box { display:flex; flex-direction:column; gap:10px; min-width:0; }
.box.row { flex-direction:row; align-items:stretch; flex-wrap:wrap; }
.box.row > * { flex:1 1 180px; }
.card { border:1px solid var(--faint); border-radius:10px; background:rgba(0,78,99,.42); padding:16px; min-width:0; }
...
.metric-value { color:var(--brand-green); font-size:30px; font-weight:800; overflow-wrap:anywhere; }
...
.status { background:var(--inner-blue); border:1px solid var(--faint); border-radius:9px; color:var(--muted); padding:9px 11px; }
.status.error { border-color:var(--danger); color:var(--danger); }
.status.warn { border-color:var(--brand-orange); }
.table-wrap { border:1px solid var(--faint); border-radius:10px; overflow:auto; }
table { border-collapse:collapse; width:100%; }
th,td { padding:10px 12px; text-align:left; white-space:nowrap; }
th { background:var(--inner-blue); color:var(--mist); font-size:11px; letter-spacing:.05em; text-transform:uppercase; }
td { border-top:1px solid var(--faint); }
tbody tr:hover { background:rgba(38,105,122,.22); }
.pill { border:1px solid var(--faint); border-radius:999px; display:inline-block; font-size:11px; font-weight:700; padding:2px 10px; }
.pill.good { background:var(--brand-green); border-color:transparent; color:#06231a; }
.pill.warn { background:var(--brand-orange); border-color:transparent; color:#2a1300; }
.pill.neutral { background:rgba(255,255,255,.08); color:var(--white); }
.bar-row { display:grid; grid-template-columns:minmax(70px,1fr) 3fr auto; gap:8px; align-items:center; }
.bar-track,.gauge { background:var(--inner-blue); border-radius:999px; height:9px; overflow:hidden; }
.bar-fill,.gauge-fill { background:var(--brand-green); height:100%; min-width:2px; }
.spark { display:flex; align-items:flex-end; gap:3px; height:48px; }
.spark span { background:var(--brand-green); border-radius:2px 2px 0 0; flex:1; min-width:3px; }
.log { border-bottom:1px solid var(--faint); display:grid; grid-template-columns:auto auto 1fr; gap:8px; padding:7px 0; }
.log-level { color:var(--brand-orange); font-weight:700; }
...
function requestFullscreen(){if(!rendered||!fullscreenSupported||fullscreenRequested)return;fullscreenRequested=true;request("ui/request-display-mode",{mode:"fullscreen"}).catch(function(){});}
...
(function(){var announced=false;function announce(){if(announced)return;announced=true;notify("ui/notifications/initialized",{});}request("ui/initialize",{appInfo:{name:"freeclimb-dashboard",version:"1.0.0"},appCapabilities:{availableDisplayModes:["inline","fullscreen"]},protocolVersion:"2025-06-18"}).then(function(result){var modes=result&&result.hostContext&&result.hostContext.availableDisplayModes;fullscreenSupported=Array.isArray(modes)&&modes.indexOf("fullscreen")!==-1;announce();requestFullscreen();}).catch(announce);setTimeout(announce,800);})();
</script>
</body>
</html>`
