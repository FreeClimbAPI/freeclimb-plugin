...
2. Builds the standalone MCP server once with `npm run setup` (install + build), producing `mcp/lib/bin.js`.
...
5. Optionally installs the `freeclimb` CLI for power users (`npm i -g ./cli`).
