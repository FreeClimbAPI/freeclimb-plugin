...
3. Connects the account via the local browser login flow (`node mcp/lib/bin.js login`). Credentials are entered and verified in a local browser page before being stored in the OS keyring, never in chat.
...
Use `/freeclimb-account` for later account status, switching, or logout without rebuilding the plugin.
